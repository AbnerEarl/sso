...
func ParseDetached(
	...
	if payload == nil {
		return nil, errors.New("sso/jose: nil payload")
	}
...
func (parsed *rawJSONWebSignature) sanitized(signatureAlgorithms []SignatureAlgorithm) (*JSONWebSignature, error) {
	if len(signatureAlgorithms) == 0 {
		return nil, errors.New("sso/jose: no signature algorithms specified")
	}
	if parsed.Payload == nil {
		return nil, fmt.Errorf("sso/jose: missing payload in JWS message")
	}
	...
	if len(parsed.Signatures) == 0 {
		...
		if !containsSignatureAlgorithm(signatureAlgorithms, alg) {
			return nil, fmt.Errorf("sso/jose: unexpected signature algorithm %q; expected %q",
		...
		if jwk != nil && (!jwk.Valid() || !jwk.IsPublic()) {
			return nil, errors.New("sso/jose: invalid embedded jwk, must be public key")
		}
...
func parseSignedCompact(
	...
	if len(parts) != 3 {
		return nil, fmt.Errorf("sso/jose: compact JWS format must have three parts")
	}

	if parts[1] != "" && payload != nil {
		return nil, fmt.Errorf("sso/jose: payload is not detached")
	}
