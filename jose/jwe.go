...
func (parsed *rawJSONWebEncryption) sanitized(
	...
	if len(keyEncryptionAlgorithms) == 0 {
		return nil, errors.New("sso/jose: no key algorithms provided")
	}
	if len(contentEncryption) == 0 {
		return nil, errors.New("sso/jose: no content encryption algorithms provided")
	}
	...
	if parsed.Protected != nil && len(parsed.Protected.bytes()) > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("sso/jose: invalid protected header: %s, %s", err, parsed.Protected.base64())
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("sso/jose: cannot sanitize merged headers: %v (%v)", err, mergedHeaders)
	}
	...
	for i, recipient := range obj.recipients {
		...
		if headers.getAlgorithm() == "" {
			return nil, fmt.Errorf(`sso/jose: recipient %d: missing header "alg"`, i)
		}
		if headers.getEncryption() == "" {
			return nil, fmt.Errorf(`sso/jose: recipient %d: missing header "enc"`, i)
		}
		err := validateAlgEnc(headers, keyEncryptionAlgorithms, contentEncryption)
		if err != nil {
			return nil, fmt.Errorf("sso/jose: recipient %d: %s", i, err)
		}
	...
	if obj.protected != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("sso/jose: protected header: %s", err)
		}
	}
	if obj.unprotected != nil {
		err := validateAlgEnc(*obj.unprotected, keyEncryptionAlgorithms, contentEncryption)
		if err != nil {
			return nil, fmt.Errorf("sso/jose: unprotected header: %s", err)
		}
	}
...
func ParseEncryptedCompact(
	...
	if len(parts) != 5 {
		return nil, fmt.Errorf("sso/jose: compact JWE format must have five parts")
	}
