...
func NewMultiEncrypter(enc ContentEncryption, rcpts []Recipient, opts *EncrypterOptions) (Encrypter, error) {
	...
	if len(rcpts) == 0 {
		return nil, fmt.Errorf("sso/jose: recipients is nil or empty")
	}
...
func (ctx *genericEncrypter) addRecipient(recipient Recipient) (err error) {
	...
	case DIRECT, ECDH_ES:
		return fmt.Errorf("sso/jose: key algorithm '%s' not supported in multi-recipient mode", recipient.Algorithm)
	}
...
func (ctx *genericEncrypter) EncryptWithAuthData(plaintext, aad []byte) (*JSONWebEncryption, error) {
	...
	if len(ctx.recipients) == 0 {
		return nil, fmt.Errorf("sso/jose: no recipients to encrypt to")
	}
...
func (obj JSONWebEncryption) Decrypt(decryptionKey interface{}) ([]byte, error) {
	...
	if len(obj.recipients) > 1 {
		return nil, errors.New("sso/jose: too many recipients in payload; expecting only one")
	}

	critical, err := headers.getCritical()
	if err != nil {
		return nil, fmt.Errorf("sso/jose: invalid crit header")
	}

	if len(critical) > 0 {
		return nil, fmt.Errorf("sso/jose: unsupported crit header")
	}
	...
	if cipher == nil {
		return nil, fmt.Errorf("sso/jose: unsupported enc value '%s'", string(headers.getEncryption()))
	}
	...
	if comp := obj.protected.getCompression(); comp != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("sso/jose: failed to decompress plaintext: %v", err)
		}
	}
...
func (obj JSONWebEncryption) DecryptMulti(decryptionKey interface{}) (int, Header, []byte, error) {
	...
	if err != nil {
		return -1, Header{}, nil, fmt.Errorf("sso/jose: invalid crit header")
	}

	if len(critical) > 0 {
		return -1, Header{}, nil, fmt.Errorf("sso/jose: unsupported crit header")
	}
	...
	if cipher == nil {
		return -1, Header{}, nil, fmt.Errorf("sso/jose: unsupported enc value '%s'", string(encryption))
	}
	...
	if comp := obj.protected.getCompression(); comp != "" {
		plaintext, err = decompress(comp, plaintext)
		if err != nil {
			return -1, Header{}, nil, fmt.Errorf("sso/jose: failed to decompress plaintext: %v", err)
		}
	}

	sanitized, err := headers.sanitized()
	if err != nil {
		return -1, Header{}, nil, fmt.Errorf("sso/jose: failed to sanitize header: %v", err)
	}
