...
func CommandServe() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:   "Launch Provider",
		Example: "provider serve config.yaml",
...
func runServe(options serveOptions) error {
	...
	storage.ConfigFilePath = configFile
	...
	logger.Infof(
		"Provider Version: %s, Go Version: %s, Go OS/ARCH: %s %s",
		version,
		runtime.Version(),
		runtime.GOOS,
		runtime.GOARCH,
	...
	if c.GRPC.Addr != "" {
		...
		api.RegisterProviderServer(grpcSrv, server.NewAPI(serverConfig.Storage, logger, version))
...
func applyConfigOverrides(options serveOptions, config *Config) {
	...
	if config.Frontend.Dir == "" {
		config.Frontend.Dir = os.Getenv("PROVIDER_FRONTEND_DIR")
	}
