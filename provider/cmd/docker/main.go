// Package main provides a utility program to launch the provider container process with an optional
...
func run(args []string, execFunc func(bool, ...string) error, whichFunc func(string) string) error {
	if args[0] != "provider" && args[0] != whichFunc("provider") {
	...
	for _, tplCandidate := range args {
		if hasSuffixes(tplCandidate, ".tpl", ".tmpl", ".yaml") {
			tmpFile, err := os.CreateTemp("/tmp", "provider.config.yaml-*")
			if err != nil {
