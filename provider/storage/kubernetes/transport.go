...
// renewTokenPeriod is the interval after which provider will read the token from a well-known file.
//
...
//	Provider uses the reasonable value between these two.
...
//	Therefore, provider needs to re-read the token from the disk after some time to be sure that it uses the valid token.
