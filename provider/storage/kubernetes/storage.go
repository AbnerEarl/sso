...
import (
	...
	"github.com/AbnerEarl/sso/provider/storage/kubernetes/k8sapi"
)
...
// registerCustomResources attempts to create the custom resources provider
...
func (cli *client) UpdateKeys(updater func(old storage.Keys) (storage.Keys, error)) error {
	...
	if firstUpdate {
		...
		if err != nil && errors.Is(err, storage.ErrAlreadyExists) {
			...
			cli.logger.Debugf("Keys creation failed: %v. It is possible that keys have already been created by another provider instance.", err)
	...
	if isKubernetesAPIConflictError(err) {
		// We need to tolerate conflicts here in case of HA mode.
		// provider instances run keys rotation at the same time because they use SigningKey.nextRotation CR field as a trigger.
		cli.logger.Debugf("Keys rotation failed: %v. It is possible that keys have already been rotated by another provider instance.", err)
