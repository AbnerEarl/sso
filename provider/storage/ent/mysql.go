...
import (
	...
	"github.com/AbnerEarl/sso/provider/storage/ent/client"
	"github.com/AbnerEarl/sso/provider/storage/ent/db"
)
...
func (m *MySQL) Open(logger log.Logger) (storage.Storage, error) {
	...
	databaseClient := client.NewDatabase(
		...
		// Set tx isolation leve for each transaction as provider does for postgres
...
func (m *MySQL) driver() (*entSQL.Driver, error) {
	...
	if m.MaxIdleConns == 0 {
		/* Override default behaviour to fix https://github.com/AbnerEarl/sso/provider/issues/1608 */
