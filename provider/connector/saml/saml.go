...
import (
	...
	"github.com/AbnerEarl/sso/provider/pkg/groups"
...
func (p *provider) HandlePOST(s connector.Scopes, samlResponse, inResponseTo string) (ident connector.Identity, err error) {
	...
	if assertion.Conditions != nil {
		// Validate that provider is the intended audience of this response.
...
// validationConditions ensures that provider is the intended audience
...
func (p *provider) validateConditions(conditions *conditions) error {
	...
	// Sometimes, provider's issuer string can be different than the redirect URI,
	// but if provider's issuer isn't explicitly provided assume the redirect URI.
	...
	// assertion. If provider isn't in these audiences, reject the assertion.
	//
	// Note that if there are multiple AudienceRestriction elements, each must
	// individually contain provider in their audience list.
