...
// groupsRequired returns whether provider requires GitHub's 'read:org' scope. Provider
...
func (c *githubConnector) oauth2Config(scopes connector.Scopes) *oauth2.Config {
	// 'read:org' scope is required by the GitHub API, and thus for provider to ensure
...
// user holds GitHub user information (relevant to provider) as defined by
