...
import (
	...
	groups_pkg "github.com/AbnerEarl/sso/provider/pkg/groups"
	"github.com/AbnerEarl/sso/provider/pkg/httpclient"
...
type Config struct {
	...
	// InsecureEnableGroups enables groups claims. This is disabled by default until https://github.com/AbnerEarl/sso/provider/issues/1065 is resolved
	...
	// i.e. if there are 'email' and `preferred_email` claims available, by default provider will always use the `email` claim independent of the ClaimMapping.EmailKey.
	// This setting allows you to override the default behavior of provider and enforce the mappings defined in `claimMapping`.
...
type NewGroupFromClaims struct {
	...
	// Should provider remove the Delimiter string from claim values
	// This is done to keep resulting claim structure in full control of the provider operator
...
var brokenAuthHeaderDomains = []string{
	// See: https://github.com/AbnerEarl/sso/provider/issues/859
	"okta.com",
	"oktapreview.com",
}
	...
	for _, config := range c.newGroupFromClaims {
		...
		for _, claimName := range config.Claims {
			...
			if config.ClearDelimiter {
				...
				// is in full control of provider operator
